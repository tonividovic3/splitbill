...
import {
  ...
  ScrollView, TextInput, ActivityIndicator, Linking, Animated
} from 'react-native'
...
export default function GuestScreen() {
  ...
  async function confirm() {
    if (!name.trim()) return
    if (session?.session_type !== 'trip' && selected.size === 0) return
    setSaving(true)

    const payload = {
      session_code: code,
      guest_name: name.trim(),
      item_ids: session?.session_type === 'trip' ? [] : [...selected],
      total: myTotal,
      tip_percent: tipPct,
      confirmed: true
    }
    ...
    <SafeAreaView style={s.safe} edges={['top', 'bottom']}>
      <Animated.View style={[s.nameContainer, { opacity: fadeAnim }]}>
        <LinearGradient colors={['#7857FF', '#4F37CC']} style={s.sessionBadge}>
          <Text style={{ fontSize: 28 }}>{isTrip ? '✈️' : '🧾'}</Text>
        </LinearGradient>
        ...
        {isTrip && (
          <View style={s.tripInfo}>
            <Text style={s.tripInfoText}>
              Trip mode · {session.trip_people} people · {fmtCurrency(session.total / (session.trip_people || 2), session.currency)} each
            </Text>
          </View>
        )}
  ...
  if (screen === 'select') return (
    <SafeAreaView style={s.safe} edges={['top']}>
      <ScrollView contentContainerStyle={s.selectScroll} showsVerticalScrollIndicator={false}>
        ...
        {isTrip ? (
          /* Trip mode — equal share, just confirm */
          <View style={s.tripCard}>
            <Text style={s.tripCardEmoji}>✈️</Text>
            <Text style={s.tripCardTitle}>Trip Mode</Text>
              ...
              {session.items.map((item, idx) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[
                      s.itemRow,
                      idx === session.items.length - 1 && s.itemRowLast,
                      isSel && s.itemRowSelected,
                    ]}
                  ...
                  >
                    ...
                    <Text style={[s.itemName, isSel && { color: C.text }]} numberOfLines={1}>{item.name}</Text>
                    {item.qty > 1 && <View style={s.qtyBadge}><Text style={s.qtyText}>×{item.qty}</Text></View>}
                    <Text style={[s.itemPrice, isSel && { color: C.text }]}>{fmtCurrency(item.price, session.currency)}</Text>
                  </TouchableOpacity>
                )
              })}
            </View>
        ...
        <TouchableOpacity
          disabled={(selected.size === 0 && !isTrip) || saving}
          onPress={confirm}
          activeOpacity={0.85}
        >
          <LinearGradient
            colors={(selected.size === 0 && !isTrip) || saving ? ['#2a2040', '#1a1530'] : ['#7857FF', '#5537EE']}
            style={s.primaryBtn}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 0 }}
          >
